...
// ---------------------------------------------------------------------------
function buscarTrilha(tecnologia, trilhas) {
...
function formatarPlanoEstudos(trilha) {
    ...
    if (trilha.promocoes) {
        ...
        linhas.push(`Desconto: ${p.desconto} | Cupom: ${p.cupom ?? "—"} | Válido até: ${p.validade ?? "—"}`);
    }
    else {
...
const NIVEIS_VALIDOS = ["Básico", "Intermediário", "Avançado"];
...
server.tool("trilha_buscar", "Busca uma trilha de estudos DIO pela tecnologia desejada e retorna o plano de estudos em Markdown.", {
...
server.tool("desafio_gerar", "Gera um desafio de código aleatório para uma tecnologia e nível escolhidos.", {
...
server.tool("certificado_gerar", "Gera um certificado fictício de conclusão para um aluno em uma trilha DIO.", {
